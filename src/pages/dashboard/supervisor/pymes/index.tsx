...
const Index: React.FC = () => {
  ...
  const viewClient = async (uuid: string) => {
    router.push(`/dashboard/supervisor/pymes/edit/${uuid}`);
  };

  const showProcesses = async (id: number) => {
    router.push(`/dashboard/supervisor/pymes/${id}/reportes`);
  };
  ...
  return (
    <>
      <div className="flex justify-between mb-8">
        <div>
          <SectionTitle title="Pymes" subtitle="Listar Pymes" />
        </div>
        <div>
          <button
            ...
            onClick={() => router.push("/dashboard/supervisor/pymes/crear")}>
            Crear Pyme
          </button>
        </div>
      </div>
      {loading ? (
        <div className="flex justify-center">
          <div className="text-gray-500 w-full text-center p-5">
            ...
            <div className="ml-2 mt-1">Cargando Pymes...</div>
          </div>
        </div>
      ) : prospects?.length === 0 ? (
        <div className="flex justify-center">
          <div className="text-gray-500 w-full text-center p-5">
            No hay Pymes todavía.
            <br></br>
            <Link legacyBehavior href="/dashboard/supervisor/pymes/crear">
        ...
        <Table>
          <Table.Head>
            ...
            <Table.HeadCell>Pyme</Table.HeadCell>
